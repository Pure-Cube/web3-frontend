...
export const ConnectWallet = observer(({ children, type }: ConnectWalletProps) => {
  ...
  return (
    <div>
      {
        type === 'profile' ? (walletStore.account ? (
          <div className="text-center">
            <div className="text-white text-lg">Account: {walletStore.account}</div>
            <div className="text-white text-base">Balance: {walletStore.balance} ETH</div>
            <button
          ...
          <div className="flex justify-center items-center text-2xl rounded-full bg-neutral-700 w-12 h-12" onClick={connectMetaMask}>
            +
          </div>
        )) : (walletStore.account ? (
